...
class ChatBot:
    def __init__(self, model_name='skt/kogpt2-base-v2', model_path='models/model.pt'):
        self.bos_token = '<s>'
        self.eos_token = '</s>'
        self.usr_token = '<usr>'
        self.pad_token = '<pad>'
        ...
        self.bad_words_ids = [self.tokenizer.encode(token) for token in [
                            self.unk_token, 'ㅋ', 'ㅠ', 'ㅠㅠ', 'ㅜㅜ', 'ㅜ', 'ㅎ', 'ㅠㅜ', 'ㅜㅠ', 'ㅋㅎ', 'ㅎㅋ', '.', '하하']]
        self.usr_token_id = self.tokenizer.convert_tokens_to_ids(self.usr_token)
        self.sys_token_id = self.tokenizer.convert_tokens_to_ids(self.sys_token)
        ...
        if result == user_message:
            with torch.no_grad():
                ...
                reply_ids = self.model.generate(
                            ...
                            eos_token_id=[self.usr_token_id, self.sys_token_id]
                        )   
